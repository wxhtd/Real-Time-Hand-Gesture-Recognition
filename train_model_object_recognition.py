...
def load_data(data_dir, gestures, max_frames=None):
    ...
    for label, gesture in enumerate(gestures):
        ...
        file_path = os.path.join(data_dir, f"{gesture}_combined2.csv")
        ...
        for batch_num in data[0].unique():
            ...
            distances = batch_data.iloc[:, 1:].values  # Distance columns
            ...
            max_frames_found = max(max_frames_found, len(distances))
...
root_path = "C:\\Users\\wxhtd\\OneDrive\\Desktop\\Final\\temp\\dynamic"
dynamic_gestures = [g.strip() for g in "thumbs_up, thumbs_down, palm_left, palm_right, palm_up, palm_down, palm_forward, palm_backward, none, cross, fist, heart, palm, stop".split(',')]
...
# Train the model
model_3d.fit(X_train_dyn, y_train_dyn, epochs=23, batch_size=8, validation_data=(X_test_dyn, y_test_dyn))
...
model_name = 'gesture_3dcnn_obj_only_20241209_f_1.h5'
